...
func ExecuteC(filename string, stdinputs []string) (comphubtypes.ExecutionRes, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()

	executable := "output_binary"
	compileCmd := exec.CommandContext(ctx, "gcc", "-o", executable, filename)
